...
/**
 * The Durable Object and local test store inherit these same named operations,
 * so tests run the real queries against the real migrations without a Workers
 * runtime or a second forwarding surface.
 *
 ...
 */
export const REPOSITORY_DATABASE = Symbol("repositoryDatabase");
export const REPOSITORY_KV = Symbol("repositoryKv");

export interface RepositoryStorage {
  readonly [REPOSITORY_DATABASE]: SyncSqliteDatabase;
  readonly [REPOSITORY_KV]: SyncKv;
}

type RepositoryStorageConstructor = new (...args: any[]) => RepositoryStorage;

/** Add the repository's named operations to either local or Durable Object storage. */
export const withRepositoryStore = <TBase extends RepositoryStorageConstructor>(Base: TBase) =>
  class RepositoryStoreMixin extends Base {
    ...
    /** Restart an incomplete import from an empty bare repository. */
      ...
      await this.#exclusive(async () => {
        await this.#db.transaction((tx) => {
          ...
          for (const ref of state.refs) {
            tx.insert(refs)
              .values({ name: ref.name, objectId: ref.oid })
              .onConflictDoNothing()
              .run();
          }
    ...
    protected async destroyStorage(remove: () => Promise<void>): Promise<void> {
      ...
      if (commands.some((command) => !isDelete(command))) {
        try {
          ...
          if (
            ...
            !(error instanceof RepositoryStorageExhaustedError)
          ) {
            throw error;
          }
...
class LocalRepositoryStorage implements RepositoryStorage {
  readonly [REPOSITORY_DATABASE]: SyncSqliteDatabase;
  readonly [REPOSITORY_KV]: SyncKv;

  constructor(db: SyncSqliteDatabase, kv: SyncKv) {
    this[REPOSITORY_DATABASE] = db;
    this[REPOSITORY_KV] = kv;
  }
}

/** The same repository operations used by the Durable Object, over local storage. */
export class RepositoryStore extends withRepositoryStore(LocalRepositoryStorage) {}

export type RepositoryObjectClient = Pick<
  RepositoryStore,
  Extract<keyof RepositoryStore, string>
> & {
